...
\noindent Ort, Datum:\hfill Unterschrift Betreuer:\\[0.5cm]
\noindent \rule{6cm}{0.9pt}\hfill \rule{6cm}{0.9pt}
\newpage
...
\begin{itemize}[topsep=0pt,itemsep=0pt,parsep=0pt,leftmargin=1.5em]
...
\begin{itemize}
  ...
  \item eine Navigationsbar mit Keyword-Bereich, Statistik- und optionalem
        Fortschrittsbereich, Wiederholungsbereich und Einstellungsbereich
\end{itemize}
...
Hier überlegte ich mir, eine Node-Hover-Ansicht einzubinden, damit beim Hovern über
einer Challenge nicht nur eine kleine Detailansicht erscheint, sondern der
Lernpfad-Kontext sichtbar wird:

\begin{itemize}
  \item Die aktuelle Challenge steht im Mittelpunkt.
  \item Links erscheint die vorherige Challenge, leicht abgedunkelt und bei
        Abschluss mit Haken-Markierung.
  \item Rechts erscheint die nächste Challenge, ebenfalls leicht abgedunkelt
        als Vorschau auf den nächsten Schritt; am Anfang eines Lernpfades ist
        nur die rechte Seite sichtbar.
  \item Über einen Button \glqq Lernpfad öffnen\grqq{} oben rechts kann eine
        vollständige Lernpfad-Ansicht geöffnet werden.
\end{itemize}
...
Außerdem plante ich einen Button \glqq Zur Nächsten\grqq{} am Ende der Challenge-Ansicht,
damit der Nutzer nicht wieder zur Lernpfad- oder Graphansicht zurückkehren muss.
...
\begin{itemize}
  \item Tags werden als Tag-Cloud dargestellt; ein Klick auf einen Tag zeigt alle
        zugehörigen Challenges sowie deren Abhängigkeiten.
\end{itemize}
...
\begin{itemize}
  \item Kanten repräsentieren \texttt{dependson}-Beziehungen zwischen Nodes und
        sollen mit einer leicht pulsierenden Animation in Richtung der Ziel-Challenge
        visualisiert werden.
  \item Zusätzlich ist eine visuelle Unterscheidung der Status Current, To-Do,
        Completed und Learning vorgesehen; ob dabei auch die Kanten angepasst
...
\begin{itemize}
  \item Alle Challenges in einer Listenansicht mit Status und einem Balken, der
        anzeigt, wie viel Prozent bereits erledigt wurden
...
\begin{itemize}
    ...
    \item Anschließend beschrieb ich die Skripte \texttt{get\_all\_challenges.sh},
     \texttt{get\_rekursiv\_challenges.sh} und \texttt{get\_subgraph\_challenges.sh}.
...
\subsection{24.06.26 - Mittwoch}

Habe weiter an der Einbindung des abgeschlossenen Teilprojekts in meine Website gearbeitet.

\begin{itemize}
    \item Den Bereich zum aktuellen Stand weiter ausgearbeitet, damit sichtbar wird, woran ich gerade arbeite und welche nächsten Aufgaben noch offen sind.
    \item Gleichzeitig habe ich begonnen, die Projektdokumentation inhaltlich neu zu strukturieren, damit sie sich stärker an einer IHK-Projektdokumentation orientiert.
    \item Dabei habe ich darauf geachtet, dass die drei Skripte und das gemeinsame Repository als eigentliches Mini-Projekt klarer im Mittelpunkt stehen.
\end{itemize}

\subsection{25.06.26 - Donnerstag}

Habe die neue Struktur der Projektdokumentation weiter ausgearbeitet und an den bisherigen Inhalt angepasst.

\begin{itemize}
    \item Geprüft, welche bereits geschriebenen Abschnitte in die neue Gliederung passen und welche thematisch umsortiert werden müssen.
    \item Die Struktur so weitergedacht, dass die Dokumentation nicht nur wie ein normaler Bericht wirkt, sondern wie ein technisches Projekt mit nachvollziehbarem Aufbau.
    \item Parallel dazu weiter an der Website gearbeitet, damit Projektdarstellung und Dokumentation inhaltlich zusammenpassen.
\end{itemize}

\subsection{26.06.26 - Freitag}

Habe die neue Dokumentationsstruktur konkret in den vorhandenen Text eingebaut.

\begin{itemize}
    \item Bestehende Inhalte aus der bisherigen Dokumentation in die neue Reihenfolge gebracht.
    \item Darauf geachtet, dass Einleitung, technische Ausgangslage, Beschreibung der Skripte, Umsetzung und Ergebnis klar voneinander getrennt sind.
\end{itemize}
...
\subsection{29.06.26 - Montag}

Habe die neue Struktur der Projektdokumentation bis zum aktuellen Stand weiter fertig ausgearbeitet.

\begin{itemize}
    \item Die Dokumentation so vorbereitet, dass sie als zusammenhängender Projekttext lesbar ist und nicht nur einzelne technische Punkte auflistet.
    \item Dabei darauf geachtet, dass die Gliederung die Arbeit mit den Skripten, dem Repository und der geplanten Weiterverwendung im Projekt sauber abbildet.
    \item Um so eine deutlich klarere Grundlage geschaffen, auf der später Betreuer-Feedback und eine echte IHK-Dokumentation aufbauen können.
\end{itemize}
